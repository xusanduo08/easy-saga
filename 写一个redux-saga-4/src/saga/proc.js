...
function proc(env, iterator, mainCb, name){
  ...
  mainCb.cancel = task.cancel; // 如果当前task是个附属task，则需要给mainCb设置cancel方法，便于父task的取消动作
  next();
  return task;
  ...
  function next(arg, isErr){
    try{
      ...
      if(arg === 'cancel_task'){
        ...
        result = is.func(iterator.return) ? iterator.return('cancel_task') : {vlaue:'cancel_task', done: true};
      } else if (isErr){
  ...
  function runEffect(effect, currCb){
    ...
    if(is.promise(effect)){
      ...
      effectRunner(env, effect.payload, currCb, {parentTask: task, digestEffect});
    } else {
      currCb(effect);
    }
  }
